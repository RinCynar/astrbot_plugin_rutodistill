...
class PromptBuilder:
    ...
    def build_prompts(profile: PersonaProfile, mode: str, opening_word_use_prob: int = 50) -> Tuple[str, str]:
        ...
        opening_word_use_prob：开场词使用概率（0-100，默认 50）。0 < prob < 100 时，
        插件在每条回复发起前自行判定该轮是否命中开场词使用（命中率 = prob%）：
        命中轮注入「允许」规则，未命中轮注入「硬性禁用」指令，保证不使用开场词的
        回复比例真实生效；100 = 每次必用（仍随机轮换），0 = 禁用。
        ...
        if has_features:
            ...
            try:
                ...
                prob = 50
            if prob <= 0:
                system_parts.append("- 开场词运用（禁用）：即使目标用户有习惯性开场词，本条回复也严禁以任何开场词开头，必须直接切入话题正文。")
            elif prob >= 100:
                system_parts.append("- 开场词运用（每次必用）：若目标用户已有习惯性开场词，本条回复必须以一个开场词开头（每次至多一个），并从已掌握的全部开场词中随机轮换、禁止总是抽中同一个。")
            else:
                # 概率制：插件在每条回复发起前自行判定本轮是否命中开场词使用，
                # 命中轮注入「允许」规则，未命中轮注入「硬性禁用」指令。
                # 不依赖模型自行按概率随机（LLM 无法真正采样概率，容易因拟态惯性过量使用），
                # 从而保证「不使用开场词、直接切入话题」的回复比例真实生效。
                if random.random() * 100 < prob:
                    system_parts.append(
                        f"- 开场词运用（本条允许，目标使用率 {prob}%）：本轮命中开场词使用。"
                        f"若目标用户已有习惯性开场词，本条回复可以以一个开场词开头（每次至多一个），"
                        f"并从已掌握的全部开场词中随机轮换、禁止总是抽中同一个。"
                        f"但若对话历史中你最近一条回复已以某个开场词开头，本条必须改为直接切入话题、不再使用开场词。"
                    )
                else:
                    system_parts.append(
                        f"- 开场词运用（本条禁止，目标使用率 {prob}%）：本轮未命中开场词使用。"
                        f"即使目标用户有习惯性开场词，本条回复也严禁以任何开场词开头，必须直接切入话题正文。"
                    )
