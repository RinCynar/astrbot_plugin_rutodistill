...
- **表情符号严格辨析（Emoji vs 纯字符颜文字/符号习惯）**：
  - 系统自带的 Unicode Emoji（黄脸/图标表情如 😊、🤔、😂、🥺 等）多为通用符号，**严禁**把输入中偶然出现的通用 Emoji 当作独特个性，更严禁脑补 Emoji；
  - 目标用户真实书写的**纯文本字符颜文字**（Kaomoji 如 `(｡•̀ᴗ-)✧`、`awa()`、`_(:з」∠)_` 等）或**独特字符语癖**（如句尾未闭合的 `（`、`www` 前后缀、ASCII 连续句点等）属于核心表达指纹，必须**逐字符保真记录**到细节库或语癖中。
- **认知与价值观深度**：提取 cognition 与 values 维度时，透过语言表层深挖其**心理底色、思维方式、决策倾向与价值取向**（如直给/委婉、批判性思辨/共情包容、幽默解构/严肃推导、对特定事物的稳定立场等），使特征兼具思维深度与行为真实感。
